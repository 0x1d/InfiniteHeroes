...
export const LoadingFX: React.FC<LoadingFXProps> = ({ status }) => {
    ...
    const getStatusText = () => {
        switch(status) {
            ...
            case 'inking': return 'INKING PANELS...';
            default: return 'INKING PAGE...';
        }
    };
    ...
    return (
        <div className="w-full h-full bg-white overflow-hidden relative border-r-4 border-gray-300">
            ...
            <div className="absolute bottom-24 inset-x-0 flex flex-col items-center gap-2 z-10">
                <p className="font-comic text-xl text-gray-800 animate-pulse tracking-widest bg-white/80 px-4 py-1 border-2 border-black rotate-[-1deg]">{getStatusText()}</p>
            </div>
        </div>
    );
};
